...
import { Package, CreditCard, Wrench, CheckCircle, TrendingUp, Calendar, AlertCircle, ShoppingBag, Activity, Users } from 'lucide-react';
import { supabase } from '../lib/supabase';
import { useAuth } from '../contexts/AuthContext';
import { formatCurrency, formatDate } from '../lib/utils';
import StatusBadge from '../components/common/StatusBadge';
import LoadingSpinner from '../components/common/LoadingSpinner';
import type { Order, Event, Profile, Service, Assignment, WorkLog } from '../lib/types';
...
export default function DashboardPage() {
  ...
  async function loadAdminDash() {
    const [ordersRes, eventsRes, paymentsRes, usersRes] = await Promise.all([
      supabase.from('orders').select('id, status, total_price, invoice_number, created_at, service:services(id, name), exhibitor:profiles!orders_exhibitor_id_fkey(id, name)').order('created_at', { ascending: false }).limit(8),
      supabase.from('events').select('*').in('status', ['published', 'active']).order('start_date').limit(4),
      supabase.from('payments').select('id').eq('status', 'pending_verification'),
    ...
    setTotalRevenue(orderList.filter((o) => ['paid', 'assigned', 'on_progress', 'completed'].includes(o.status)).reduce((s, o) => s + o.total_price, 0));
  }
  ...
  async function loadEoAdminDash() {
    const [ordersRes, eventsRes, assignRes, paymentsRes] = await Promise.all([
      supabase.from('orders').select('id, status, total_price, invoice_number, created_at, service:services(id, name), exhibitor:profiles!orders_exhibitor_id_fkey(id, name)').order('created_at', { ascending: false }).limit(8),
      supabase.from('events').select('*').in('status', ['published', 'active']).order('start_date').limit(4),
      supabase.from('assignments').select('id, order_id, contractor:profiles!assignments_contractor_id_fkey(id, name)').order('created_at', { ascending: false }).limit(5),
      supabase.from('payments').select('id').eq('status', 'pending_verification'),
    ]);
    setOrders((ordersRes.data as Order[]) ?? []);
    setEvents((eventsRes.data as Event[]) ?? []);
    setAssignments((assignRes.data as AssignmentWithLog[]) ?? []);
    setPendingPaymentsCount(paymentsRes.data?.length ?? 0);
    const orderList = (ordersRes.data as Order[]) ?? [];
    setTotalRevenue(orderList.filter((o) => ['paid', 'assigned', 'on_progress', 'completed'].includes(o.status)).reduce((s, o) => s + o.total_price, 0));
  }
  ...
  async function loadExhibitorDash() {
    const [ordersRes, eventsRes] = await Promise.all([
      supabase.from('orders').select('id, status, total_price, invoice_number, created_at, service:services(id, name)').eq('exhibitor_id', profile!.id).order('created_at', { ascending: false }).limit(8),
      supabase.from('events').select('*').in('status', ['published', 'active']).order('start_date').limit(4),
    ]);
    setOrders((ordersRes.data as Order[]) ?? []);
    setEvents((eventsRes.data as Event[]) ?? []);
  ...
  async function loadContractorDash() {
    const assignRes = await supabase
      .from('assignments')
      .select('*, order:orders(id, invoice_number, service:services(id, name), booth:booths(id, number))')
      .eq('contractor_id', profile!.id)
      .order('created_at', { ascending: false })
      .limit(8);

    const list = (assignRes.data as Assignment[]) ?? [];
    const ids = list.map((a) => a.id);
    const logsMap: Record<string, WorkLog> = {};
    if (ids.length > 0) {
      const { data: logs } = await supabase.from('work_logs').select('*').in('assignment_id', ids).order('created_at', { ascending: false });
      (logs ?? []).forEach((l) => { if (!logsMap[l.assignment_id!]) logsMap[l.assignment_id!] = l as WorkLog; });
    }
  ...
  const activeAssignments = assignments.filter((a) => (a.latest_log?.status ?? 'waiting') !== 'complete').length;
  const doneAssignments = assignments.filter((a) => a.latest_log?.status === 'complete').length;

  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-bold text-gray-900 dark:text-white">Welcome back, {profile?.name?.split(' ')[0]}</h2>
      ...
      {(role === 'super_admin' || role === 'eo_admin') && (
          ...
          <div className="lg:col-span-2 bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 p-5">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-base font-semibold text-gray-900 dark:text-white">
                {role === 'exhibitor' ? 'My Recent Orders' : 'Recent Orders'}
              </h3>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-gray-100 dark:border-gray-800">
                      {['Invoice', 'Service', role !== 'exhibitor' ? 'Exhibitor' : null, 'Total', 'Status'].filter(Boolean).map((h) => (
                  ...
                  <tbody className="divide-y divide-gray-50 dark:divide-gray-800">
                    {orders.map((o) => (
                      <tr key={o.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                        ...
                        {role !== 'exhibitor' && <td className="py-2.5 pr-4 text-gray-600 dark:text-gray-300 whitespace-nowrap">{(o.exhibitor as unknown as Profile)?.name ?? '-'}</td>}
